...
namespace NT_CommonConfig.Infrastructure
{
    public class WeChatProfile : Profile
    {
        ...
        {
            this.CreateMap<WeChatUserBasicInfo, WeChatInfo>().ForMember(p => p.TagIdList, opt => opt.MapFrom(p => p.Tagid_List != null ? string.Join(",", p.Tagid_List) : string.Empty)).ForAllMembers(p => p.Condition((s, d, sm, dm) => sm != null));
